...
- Validación de body, query params y params con Zod.
...
packages/route-forge/   Biblioteca y pruebas unitarias
tests/e2e/              Pruecas E2E con Playwright
```
...
| `pnpm build`         | Compila las apps y paquetes                  |
...
- `apps/test-app` usa la versión estable actual de Next.js y consume `@repo/route-forge` mediante `workspace:*`.
- Playwright ejecuta siete casos E2E de API sin descargar navegadores.
- El build de la app valida que Next pueda transpilar el código TypeScript del paquete.
